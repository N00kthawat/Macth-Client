...
const ExplorePage: React.FC = () => {
  ...
  const handleOpen = async (hotel: Hotel) => {
    ...
    setOpenModal(true);
    try {
      const res = await axios.get(`${API_URL}/hotels/${hotel.id}/rooms`);
      setRooms(res.data);
    } catch (error) {
            ...
            <div className="sticky top-0 z-10 bg-white/90 backdrop-blur-sm px-4 md:px-6 pt-4 pb-2 border-b border-gray-200 flex justify-between items-start">
              {selectedHotel && (
                <>
                  ...
                  <button
                    onClick={handleClose}
                    className="text-xs md:text-sm text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-indigo-500 hover:opacity-80 transition"
                  >
                    ✕ ปิด
                  </button>
                </>
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {filteredRooms.map((room) => (
                      <div
                        key={room.id}
                        className="border border-gray-200 p-3 rounded-xl shadow-sm hover:shadow-md transition bg-white"
                      >
